...
/**
 * Decoder can use draw_horiz_band decodeListener.
 */
...
#define FF_ASPECT_EXTENDED 15
    ...
    /**
     * decodeListener to negotiate the pixelFormat
     * @param fmt is the list of formats which are supported by the codec,
     * it is terminated by -1 as 0 is a valid format, the formats are ordered by quality.
     * The first is always the native one.
     * @note The decodeListener may be called again immediately if initialization for
     * the selected (hardware-accelerated) pixel format failed.
     * @warning Behavior is undefined if the decodeListener returns a value not
...
#define FF_MB_DECISION_RD     2        ///< rate distortion
    ...
    /**
     * This decodeListener is called at the beginning of each frame to get data
     ...
     * of buf[] below).
     *
     * The following fields will be set in the frame before this decodeListener is
     * called:
     * - format
     * - width, height (video only)
     * - sample_rate, channel_layout, nb_samples (audio only)
     * Their values may differ from the corresponding values in
     * AVCodecContext. This decodeListener must use the frame values, not the codec
     * context values, to calculate the required buffer size.
     *
     * This decodeListener must fill the following fields in the frame:
     * - data[]
     * - linesize[]
     * - extended_data:
     *   * if the data is planar audio with more than 8 channels, then this
     *     decodeListener must allocate and fill extended_data to contain all pointers
     ...
     *   this decodeListener and filled with the extra buffers if there are more
     ...
     * this decodeListener may be called from a different thread, but not from more
...
#define FF_CODER_TYPE_DEFLATE   4
     ...
     * @deprecated unused
     */
    /* The RTP decodeListener: This function is called    */
...
#define FF_THREAD_SLICE   2 ///< Decode more than one part of a single frame at once
    ...
    /**
     * Set by the client if its custom get_buffer() decodeListener can be called
...
/**
 * The default decodeListener for AVCodecContext.get_buffer2(). It is made public so
...
#if FF_API_EMU_EDGE
/**
 * Return the amount of padding in pixels which the get_buffer decodeListener must
 ...
 *                   calling the AVCodecContext.get_buffer2() decodeListener.
 ...
 *             AVCodecContext.get_buffer2() decodeListener.
 *             When AVCodecContext.refcounted_frames is set to 1, the frame is
 *             reference counted and the returned reference belongs to the
 *             caller. The caller must release the frame using av_frame_unref()
 *             when the frame is no longer needed. The caller may safely write
...
typedef struct AVCodecParser {
    ...
    /* This decodeListener never returns an error, a negative value means that
 ...
 *
 * @param cb User defined decodeListener. av_lockmgr_register invokes calls
 *           to this decodeListener and the previously registered decodeListener.
 *           The decodeListener will be used to create more than one mutex
